...
export default function AppShell({ routeSection, authRoute }) {
  ...
  const [activeSection, setActiveSection] = useState(routeSection || "dashboard");
  ...
  useKeyboardShortcut("n", () => setShowFindingModal(true), {
    enabled: activeSection === "hallazgos" && Boolean(projectId),
  });
  ...
  useKeyboardShortcut("n", () => openNewAssetModal(), {
    enabled: activeSection === "activos" && Boolean(projectId),
  });
  useKeyboardShortcut("n", () => {
    setShowScanWizard(true);
    setWizardStep(1);
  }, {
    enabled: activeSection === "escaneos" && Boolean(projectId),
  });
  useKeyboardShortcut("n", () => {
    setShowUserModal(true);
    setUserModalTab("invite");
  }, {
    enabled: activeSection === "equipo" && Boolean(projectId),
  });
  ...
  useEffect(() => {
    if (user) {
      setRequiresProfile(Boolean(user.profile_completed === false));
    } else {
      setRequiresProfile(false);
    }
  }, [user]);
  ...
  }, [requiresProfile, handleSectionChange]);
  ...
  useEffect(() => {
    ...
    if (authRoute === "register") {
      ...
      setAuthMode("login");
      setResetMode(false);
    } else if (authRoute === "forgot-password" || authRoute === "reset-password") {
      setAuthMode("login");
      setResetMode(true);
    } else if (authRoute === "rotate-password") {
  ...
  }, [projectId, isAuthenticated, requiresProfile, reloadToken]);
  ...
  }, [orgId, isAuthenticated, requiresProfile, reloadToken]);
  ...
  }, [orgs, projectId, isAuthenticated, requiresProfile, reloadToken]);
  ...
  }, [isAuthenticated, requiresProfile, reloadToken]);
          ...
          {error ? (
            <div className="error-banner" role="alert">
              ...
              <div className="error-banner__actions">
                ...
                <button className="btn btn-ghost" type="button" onClick={() => setError("")}>
                  Cerrar
                </button>
              </div>
            </div>
              ...
              {activeSection === "perfil" && (
                error ? (
                  <EmptyState
                    ...
                    description={`Intenta nuevamente o revisa tu conexión. ${error}`}
